...
class PsychoacousticModel:
    ...
    def maskingThreshold(self, mX):
        ...
        for n in range(Numsubbands):
            for m in range(0, n):
                mT[n] += (mX[m] * self._fa * (self._fb ** ((n - m) * fc))) ** self._alpha

            for m in range(n+1, Numsubbands):
                mT[n] += (mX[m] * self._fa * (self._fbb ** ((m - n) * fc))) ** self._alpha
    ...
    def NMREval(self, mX1, mX2):
        ...
        # NMR computation
        NMR = 10. * np.log10((1./mX1.shape[0]) * self._maxb * np.sum((imT * (Err*LTq))))
        return NMR
